...
Wordmark **1.5.2**. Changelog UI does not own `SAVE_VERSION`, dump `version`, or `PROTOCOL`. No migrate.
...
`RELEASES` spans Beta-1 through the current wordmark `1.5.2`. Array order is render order. `id` is the version key the copy owner names. Duplicate `id` illegal.
...
Open: click the existing wordmark `<p>1.5.2</p>` (boot and play). Sets `{ kind: 'changelog' }`. Toggle: click again → `{ kind: 'home' }`. While boot `joining`: no-op.
