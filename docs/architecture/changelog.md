...
Wordmark **1.7.2**. Changelog UI does not own `SAVE_VERSION`, dump `version`, or `PROTOCOL`. No migrate.

Assumption: wordmark **1.7.2**; this note does not invent 1.7.2 `RELEASES` copy.
...
`RELEASES` spans Beta-1 through the current wordmark `1.7.2`. Array order is render order. `id` is the version key the copy owner names. Duplicate `id` illegal.
...
Open: click the existing wordmark `<p>1.7.2</p>` (boot and play). Sets `{ kind: 'changelog' }`. Toggle: click again → `{ kind: 'home' }`. While boot `joining`: no-op.
