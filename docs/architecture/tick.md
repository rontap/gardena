...
`evalSensors` storeRaw and `padBuildings` also walk existing World silo / additive instance lists (`silo`, `additives`, `seedSilos`). View props for house / truck / pumps / taps / wells / hangars / field silos come from those World lists; ground stays terrain.
