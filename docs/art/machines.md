...
1×1 tile viewBox except still 2×1, furnace / weather station 1×2, mill / infuser / necronomicon 2×2. Occupancy stays 2×1 / 1×2 / 2×2. Drawn still is 1.5×1 (`36×24`) centered (`x=6`–`42`). Drawn furnace is 1×1.5 (`24×36`) south-aligned (`y=12`–`48`). Drawn weather station is 0.75×2 (`18×48`) centered (`x=3`–`21`), full height. Drawn infuser is 1.5×1.75 (`36×42`) south-centered (`x=6`–`42`, `y=6`–`48`). Item face is the same machine as the prop, larger — do not shrink the item. Cottage machines: copper reads as roof. Cold / condenser / litre marks / glass tubes read as water. Still: industrial steel onion, iron lyne arm, oil foot. No `#8b3a2a` on the pot. Condenser stays water. Furnace: iron / oil stone, roof brick, steel lip; opening on the south face of the south cell. Chimney on the north of that body, in the north cell. Chimney mouth `(12, 14)`. Infuser: two tall glass columns on an iron bench, copper arch between their caps, copper serpentine coil east, dial on a post west, pump column between the columns, two fireboxes under the bench. Groups `off` `on`. Roof token is the arch, the collars and the coil, not a rectangular outline. No full-height brown walls. Column interiors are semi-transparent glass the ground reads through, and the liquid sits over that glass.

## Buildings

| file | depicts |
|---|---|
| `prop-mill.svg` | 2×2 four-course tapered `house` tower on a plinth, `roof` cap, hub and four-arm sail cross in front, door at the foot |
| `item-mill.svg` | same windmill packed into 24×24, card/hand |
| `prop-still.svg` | 2×1 drawn 1.5×1 centered, steel onion + iron lyne, water condenser, litre marks |
| `item-still.svg` | same machine packed into 24×24, card/hand |
...
| `prop-furnace.svg` | 1×2 drawn 1×1.5 south-aligned, iron / oil body, roof brick, steel lip, opening south, chimney north; groups `off` `on`; chimney mouth `(12, 14)` |
| `item-furnace.svg` | same machine, card/hand, stays 24×24 |
| `prop-weather-station.svg` | 1×2 drawn 0.75×2 centered (`x=3`–`21`), full 48 tall, industrial steel / iron / oil mast, instrument on top, water window on the face. Same instance both cells. No groups |
| `item-weather-station.svg` | same mast, card/hand, stays 24×24 |
| `prop-research-station.svg` | 2×1 cottage seed-tray cabinet west, potting bench with three cuttings east, roof lamp; groups `off` `on`. Iron lamp face is the only metal |
| `item-research-station.svg` | same machine at 1×1, cabinet and one potted cutting, card/hand; groups `off` `on` |
| `prop-infuser.svg` | 2×2 drawn 1.5×1.75 south-centered, two glass columns (`x=12`–`20` and `x=28`–`36`, `y=10`–`36`) with copper collars, stepped copper arch between the caps, copper serpentine coil east, dial on a post west, iron pump column and cross pipes between, iron bench and cabinet with two oil fireboxes; groups `off` `on`. No rectangular roof bar. No full-height brown walls |
...
| file | groups | depicts |
|---|---|---|
| `fruit-sugar-cane.svg` | `common` `heirloom` | tied cane sheaf. Common leaf; heirloom leaf + ripe bands. `rare` unused: `common` stands in |
...
| file | groups | depicts |
|---|---|---|
| `item-wine.svg` | `base` `heirloom` | long-neck bottle. `base` grape; `heirloom` ripe |
| `item-cider.svg` | `base` `heirloom` | squat bottle, ripe fill |
...
Jar = skill-jam language. Ketchup is a bottle, not a jar. Passata is a can, not a jar.
...
| file | groups | depicts |
|---|---|---|
| `crop-chilli.svg` | `sprout` `grow` `dead` `ripe` | bush, hanging pointed fruit-red chillies. One Variety; ripe group is `ripe` only |
| `item-chilli-flakes.svg` | — | corked glass jar, dirt cork, fruit-red flakes through empty glass, roof bits. Not a jam jar. Not a dish |
