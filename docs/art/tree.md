...
Groups: `trunk` `grow`, then mature `unripe` `unripe-variant` `unripe-variant-2` `unripe-heirloom` and `ripe` `ripe-variant` `ripe-variant-2` `ripe-heirloom`.
...
| species | `ripe` | `ripe-variant` | `ripe-variant-2` | `ripe-heirloom` |
|---|---|---|---|---|
| apple | 4×4 fruit-red | 3×3 roof, clustered (`kingston-black`) | 4×4 ripe | 3×5 blush, tall (`pink-lady`) |
| apricot | 4×4 ripe | 5×5 ripe, fewer (`moorpark`) | 3×3 blush, more (`klosterneuburger`) | 4×4 ripe + blush cheek (`blenheim`) |
| olive | 4×4 dirt-dark | 3×5 grape (`kalamata`) | 3×3 ripe, many (`arbequina`) | 4×4 house, pit |
| cherry | 3×3 fruit-red | 3×3 fruit-red, more (`montmorency`) | 3×3 ripe | 4×4 roof, fewer (`bing`) |
...
Fruit icons `fruit-{apple,apricot,olive,cherry}.svg` are 24×24 with `base` `variant` `variant-2` `heirloom`.
