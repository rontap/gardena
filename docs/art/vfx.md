...
Groups `f0`…`fN`. Tile viewBox [[art/svg]] except spray `0 0 48 48`, spray-large `0 0 96 96`, spray-vert `0 0 96 48`, steam `0 0 48 24`.

| file | frames | is |
|---|---|---|
| `vfx-spray.svg` | `f0`–`f3` | fine mist on ten rays, droplets marching outward, reaching ±1 tile |
| `vfx-spray-large.svg` | `f0`–`f3` | same mist on fourteen rays, ±2 tiles |
| `vfx-spray-vert.svg` | `f0`–`f1` | three-row bar mist marching out both ways |
| `vfx-brew.svg` | `f0`–`f3` | bubbles off a filling cask |
| `vfx-dust.svg` | `f0`–`f1` | crush dust at the mill and the jam pot |
| `vfx-steam.svg` | `f0`–`f3` | steam off the still |
| `vfx-dig.svg` | `f0`–`f3` | clods off the spade |
| `vfx-grind.svg` | `f0`–`f3` | `leaf` / `grass-dark` chips flicking off the grinder drum, east side |
| `vfx-station.svg` | `f0`–`f3` | the station's two seed-tray bars lighting in turn, `leaf` and `ripe`, with a lamp pulse |
| `vfx-exhaust.svg` | `f0`–`f3` | `oil` / `steel` / `house` puffs thinning as they rise, behind a moving tractor |
| `vfx-burrow-pop.svg` | `f0`–`f2` | `ripe` rays bursting from a `house` core: a tight spark, a full star reaching the cell edge, then a faded ring |
| `vfx-tend.svg` | `f0`–`f1` | leaf ticks rising |
| `vfx-pour.svg` | `f0`–`f1` | splash landing |
| `vfx-furnace.svg` | `f0`–`f3` | fire at the furnace opening |
| `vfx-furnace-smoke.svg` | `f0`–`f3` | smoke rising from the furnace chimney |
| `vfx-graft.svg` | `f0`–`f3` | wood chips and leaf ticks rising off the cut |
| `vfx-age.svg` | `f0`–`f3` | bubbles rising off an aging cask, denser than `vfx-brew` |

`vfx-spray-large` is its own file, not `vfx-spray` at `scale(2)` — a scaled copy doubles the pixel grid.

**The spray does not sweep.** Four frames cannot turn an arc without strobing. Mist: droplets on fixed rays, each frame advancing them one step outward, so the motion reads as spraying and the shape never moves. Droplets are 1 unit past the head (2 near it), carry `fill-opacity` falling from 0.6 to 0.3 with distance, and the far tip is `house` at 0.35.
...
Working furnace: `vfx-furnace` mounts at the south cell — the opening. `vfx-furnace-smoke` mounts at the north cell — the chimney. Prop groups `off` / `on` light that opening; fire VFX is the fire; smoke VFX is the chimney. Mouth [[art/machines]]. Smoke `cell` origin at the north cell corner, puffs leave that mouth toward y=0. Reduced motion: frame 0 on both. Fire: `fire` / `ripe` / `fruit-red` / `roof`. Smoke: `steel` / `house` / `oil` / `ink`. No `fire` on the smoke.

Mill sails and pump arm are `flow` on the Pixi ticker, not frames — [[architecture/view]] `view.prop-motion`. A working mill also mounts `dust` at the door in the plinth.

## State: grinder, station

`grinderWorking(c)` is `crop !== 'none' && units > 0 && progress < 1`; `stationWorking` already existed. Both are base-cell guarded like mill / jam / still / barrel, so the 2×1 station mounts once.
