...
`silo-seed` — `buy-silo-seed`. `silo-spray` — `buy-silo-spray`. `silo-produce` — `buy-silo-produce`. 2×3, door south, `siloPad`. Automation, `unlock-vehicles`, `machine-contracts`. Inert: look name only, no dialog, no merge, `isSolid`, delete always. Guest `GUEST_BUILD`. Rules: [[mechanics/vehicles]].
