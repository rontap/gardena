...
`station` — `buy-research-station`. 2×1, place like the still. Processing shelf, unlock and show `unlock-crop-variants`, `need: []`. No research row of its own. `haggling`. Guest shop + place + delete + dump. `dest(station)` = `at`. Pads, west pull, east push, `inn`, like the still. Heirloom fruit in, cut fruit and grafts out. Panel [[ui/station]]. Rules: [[mechanics/machines]] `station.io`.
