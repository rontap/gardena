...
`PACK_N` packs: `pack-carrot` `pack-potato` `pack-wheat` `pack-tomato` `pack-grape` `pack-raspberry` `pack-sugar-cane` `pack-chilli`. `pack-grass` is `GRASS_PACK`, `'base'` quality 0, `{ kind: 'seeds'; crop: 'grass' }`. Prices `SKUS`. Bought packs are `'base'` at quality 0. Tomato grape via plants research: `pack-tomato` / `pack-grape` show `start`, buy after that row. Packs are not free on day 1. Raspberry after grape. Sugar cane after `unlock-fermentation`. `pack-chilli` show and buy `unlock-infusion`, `PACK_N` at 10. `pack-grass` show `start`, buy `unlock-landscaping`. Vanilla and trees have no pack. Vanilla contract prize is 1 or 2 seeds on the two 4-star cells only, `'base'` quality 0. Named (`variant`) and Heirloom (`heirloom`) tree seeds are contract prizes, quality 0. Plain (`'base'`) tree seeds still include the starting four and these boards. `halbert-eijn` and `intercrop` never pay tree seeds. Named annual contract prizes are `isAnnualId` crops that have a `variant` row. [[mechanics/contracts]]. No `pack-olive`. No `pack-vanilla`. No `unlock-chilli`. No `pack-watermelon`.
