...
Solo App: on `clock.day` increment, `writeSlot` and close the open panel. No pause — the seam does not stop the farm. Recap popup uses the same overlay pause as Family / Market / Almanac. [[ui/hud]] [[ui/settings]]
