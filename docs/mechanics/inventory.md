...
House: 16 slots. Walk to the door, swap with hand. Auto-merge same crop+rarity seeds and fruit. Sugar merges weighted `unitSale`. Weighted freshness / `unitSale` on fruit.
...
One kind: fruit or seeds, one crop+rarity. Harvest and pickup fill the box if it accepts. Not sugar. Not saplings.
...
Sugar composts as fruit (5). Empty-hand weeds/grass are feedstock. Shovel dead/rotten drops nothing — [[mechanics/plants]].

## Grind

Seed grinder 1×1, $30, `unlock-grinder`. One annual fruit (not sugar-cane, not `TreeId`) → `GRIND_MIN`..`GRIND_MAX` (1–3) seeds, same crop and rarity. `GRIND_WORK = 2` s per fruit — preference. Box dumps all accepted fruit in it. Seeds merge into house or drop if full. Tree fruit and sugar: refuse.
