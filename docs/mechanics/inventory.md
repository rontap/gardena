...
Compost box $20. `unlock-compost` $10 / 30s, plants tree, no prerequisite — priced level with `unlock-fertilizer`. `COMPOST_NEED = 10` units → one bag in `COMPOST_SECONDS = 90` — preference. Output drop on a plot in front. Dump all legal until dest full. Pads; no port. Guest dump / Load / Unload. Keep `frontOf`.
