...
`skuPrice`: drought, `tab === 'seeds' | 'utility'`, then ×2. Automation / building / hangar-buys untouched. — [[mechanics/family]]
...
`weather.market` — `marketOpen` is always true; no weather close; no phase hours; consign always.
