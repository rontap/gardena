...
Only the orchestrator may touch [[GLOBAL_VERSION]], wordmark, `SAVE_VERSION`, dump `version`, `PROTOCOL`, `src/game/ui/changelog.md`, or `changelogs-*.md`, unless the user explicitly allows a named child. Quote that allow in the child's prompt. Copy still follows [[standards/update-notes]]. Children never write a version number. Children never invent player wording.
