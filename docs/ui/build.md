...
| Automation | Grinding grinder, mill · Brewing still, barrel · Preserving jam · Compost compost-box, furnace · Grafting station · Hangar `buy-hangar` | build | Machines that make goods, and the hangar your vehicles come home to. |
...
Every sku sits in exactly one shelf group, except the eleven the stores sell (`pack-carrot` `pack-potato` `pack-wheat` `pack-tomato` `pack-grape` `pack-raspberry` `pack-sugar-cane` `buy-fertilizer` `buy-synth-fertilizer` `buy-weed-spray` `buy-sugar`) and `buy-and` `buy-or` `buy-water-system`, which sit in none — [[items/sensors]] [[ui/store]].
