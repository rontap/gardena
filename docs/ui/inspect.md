...
`inspect.open` — Six acts carry the thing's own name as their prompt text rather than a verb: `inventory` `chest` `silo` `additives` `hangar` `vehicle`. The action block wraps those in **Open {name}**, so a chest reads **Click to Open Chest**, not **Click to Chest**. `station` joins them when `canStation` is false — the same gate `begin` uses to open the panel instead of cutting — and then reads **Open Crop Variety Station**, dropping the live detail `stationLook` puts in the look block. `necronomicon` stays out: **Read the Necronomicon** is already a verb. The wrap lives in the action block alone. The prompt strings, `maybeSay`, and the queued errand rows are untouched.
