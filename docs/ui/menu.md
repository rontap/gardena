...
Top of the column: `ui-menu.svg` `w-full` (illustration only — no text in the SVG). Then `h1` **Gardena** (`font-display` `text-base` `leading-none` `text-center`). Then wordmark **1.7.2** — [[ui/changelog]]. Then the body. Then fail copy if any (home only).
...
Wordmark **1.7.2** is a `button`, `cursor-pointer`, `aria-label="Version history"`. Rest `text-sm text-ink/45 text-center px-2 py-0.5`. Hover `hover:bg-dirt hover:text-house`. While changelog: `bg-ink text-house`, `aria-pressed`. Click toggles home ↔ changelog. While joining: no-op.
