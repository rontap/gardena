...
Automation: pumpjack, well, rain-tank, tap, pipe, valve, sprinklers, chest, grinder, compost-box, then `buy-mill` `buy-still` `buy-barrel` `buy-jam` `buy-freezer` `buy-hangar`. Labels **Mill** **Pot still** **Wine barrel** **Jam machine** **Freezer** **Vehicle hangar**. `buy-hangar` show `unlock-irrigation`, buy `unlock-vehicles`, `skuPrice` (contracts). Place path. Disarm. Quad is hangar-buy, not a shop SKU. `buy-sugar` is Utility, label **Sugar**. Hangar / Quad: [[ui/vehicles]].
