...
| `text-4xl` | 32 | day banner. Line-height 1.2 |
| `text-5xl` | 48 | map boot **Loading...**. Line-height 1.2 |

Faces:

- `--font-display` **Press Start 2P**. Window titles, the **Gardena** wordmark, family member names, dash fuel and speed readouts, tractor hitch `{used}/100`, the held name and the first look line in bottom-right `Status`, the day banner on the play field, map boot **Loading...**. Nothing else. Never body copy — it does not wrap or scan.
