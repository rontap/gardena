...
| `text-sm` | 13 | secondary and meta: tab labels, stat rows, blurbs, footers, queue, Build card labels and their price |
| `text-base` | 15 | body default, row labels, buttons |
| `text-lg` | 17 | money in the top ribbon, close glyph |
| `text-xl` | 20 | crash screen body — [[ui/error-boundary]] |
| `text-2xl` / `text-3xl` | 24 / 28 | reserved |
| `text-4xl` | 32 | day banner, crash screen **Oops**. Line-height 1.2 |
| `text-5xl` | 48 | map boot **Loading...**. Line-height 1.2 |

Faces:

- `--font-display` **Press Start 2P**. Window titles, the **Gardena** wordmark, family member names, dash fuel and speed readouts, tractor hitch `{used}/100`, the held name and the first look line in bottom-right `Status`, the day banner on the play field, map boot **Loading...**, the crash screen **Oops**. Nothing else. Never body copy — it does not wrap or scan.
