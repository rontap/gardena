import {
    AND_PRICE,
    BUTTON_PRICE,
    COUNTER_PRICE,
    LAMP_PRICE,
    LEVER_PRICE,
    NOT_PRICE,
    OR_PRICE,
    PULSER_PRICE,
    SENSOR_DAY_PRICE,
    SENSOR_FERT_PRICE,
    SENSOR_HARVEST_PRICE,
    SENSOR_WATER_PRICE,
    SMART_VALVE_PRICE,
    VEHICLE_DETECTOR_PRICE,
    WATER_SYSTEM_PRICE,
} from './items.ts'
import type {ResearchId, SkuId} from '../sim/ids.ts'

export type ResearchGate = { kind: 'none' }

export type ResearchDef = {
    id: ResearchId
    name: string
    tree: 'plants' | 'utilities' | 'expansion' | 'automation'
    cost: number
    seconds: number
    reveal: 'start' | ResearchId
    requires: ResearchId | 'none'
    gate: ResearchGate
    grants: readonly string[]
    blurb: string
    effect:
        | { kind: 'unlock-sku'; sku: SkuId }
        | { kind: 'expand' }
        | { kind: 'feature' }
}

export const RESEARCH: { readonly [K in ResearchId]: ResearchDef } = {
    'unlock-tomato': {
        id: 'unlock-tomato',
        name: 'Tomato seeds',
        tree: 'plants',
        cost: 7,
        seconds: 30,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Tomato seeds in the general store.',
        effect: {kind: 'unlock-sku', sku: 'pack-tomato'},
    },
    'unlock-grape': {
        id: 'unlock-grape',
        name: 'Grape seeds',
        tree: 'plants',
        cost: 10,
        seconds: 40,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Grape seeds in the general store.',
        effect: {kind: 'unlock-sku', sku: 'pack-grape'},
    },
    'unlock-raspberry': {
        id: 'unlock-raspberry',
        name: 'Raspberry seeds',
        tree: 'plants',
        cost: 12,
        seconds: 45,
        reveal: 'unlock-grape',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Raspberry seeds in the general store.',
        effect: {kind: 'unlock-sku', sku: 'pack-raspberry'},
    },
    'unlock-watermelon': {
        id: 'unlock-watermelon',
        name: 'Watermelon seeds',
        tree: 'plants',
        cost: 8,
        seconds: 35,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Watermelon seeds in the general store.',
        effect: {kind: 'unlock-sku', sku: 'pack-watermelon'},
    },
    'unlock-heirloom': {
        id: 'unlock-heirloom',
        name: 'Heirloom crops',
        tree: 'plants',
        cost: 120,
        seconds: 120,
        reveal: 'unlock-fertilizer',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Heirloom rarity column in the store'],
        blurb: 'Unlocks Őstermelő. Heirloom produce can sell for more.',
        effect: {kind: 'feature'},
    },
    'unlock-better-tools': {
        id: 'unlock-better-tools',
        name: 'Better gardening tools',
        tree: 'utilities',
        cost: 16,
        seconds: 45,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Better shovel and Large bucket in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-better-shovel'},
    },
    'unlock-large-box': {
        id: 'unlock-large-box',
        name: 'Fruit boxes',
        tree: 'utilities',
        cost: 17,
        seconds: 50,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Large fruit box in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-box-large'},
    },
    'unlock-irrigation': {
        id: 'unlock-irrigation',
        name: 'Irrigation',
        tree: 'automation',
        cost: 20,
        seconds: 50,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Pumpjack in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-pumpjack'},
    },
    'unlock-vehicles': {
        id: 'unlock-vehicles',
        name: 'Vehicles',
        tree: 'automation',
        cost: 32,
        seconds: 70,
        reveal: 'unlock-irrigation',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Quad, tractor and trailer at the hangar'],
        blurb: 'Unlocks Hangar, tractor, trailers, and field silos. Buy Quads and tractors at a hangar.',
        effect: {kind: 'unlock-sku', sku: 'buy-hangar'},
    },
    'unlock-auto-irrigation': {
        id: 'unlock-auto-irrigation',
        name: 'Automated irrigation',
        tree: 'automation',
        cost: 20,
        seconds: 45,
        reveal: 'unlock-irrigation',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Pipe, Sprinkler, Manual valve, Rainwater tank and Tap in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-pipe'},
    },
    'unlock-adv-irrigation': {
        id: 'unlock-adv-irrigation',
        name: 'Advanced irrigation',
        tree: 'automation',
        cost: 28,
        seconds: 65,
        reveal: 'unlock-auto-irrigation',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Well, Vertical sprinkler, and Large sprinkler in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-well'},
    },
    'unlock-smart-sprinkler': {
        id: 'unlock-smart-sprinkler',
        name: 'Smart sprinklers',
        tree: 'automation',
        cost: 30,
        seconds: 70,
        reveal: 'unlock-adv-irrigation',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Sprinkler crop dial'],
        blurb: 'Every sprinkler gains a crop dial. Tuned to a crop, it pours exactly what that crop drinks.',
        effect: {kind: 'feature'},
    },
    'unlock-sensors': {
        id: 'unlock-sensors',
        name: 'Sensors',
        tree: 'automation',
        cost: 24,
        seconds: 55,
        reveal: 'unlock-auto-irrigation',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Sensors lens row'],
        blurb: 'Unlocks the Sensors shelf: lever, button, lamp, pulser, counter, and field readers.',
        effect: {kind: 'feature'},
    },
    'unlock-advanced-sensors': {
        id: 'unlock-advanced-sensors',
        name: 'Advanced sensors',
        tree: 'automation',
        cost: 22,
        seconds: 50,
        reveal: 'unlock-sensors',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks AND, OR, and NOT. AND is on if both inputs are. OR if either is. NOT inverts.',
        effect: {kind: 'feature'},
    },
    'unlock-smart-irrigation': {
        id: 'unlock-smart-irrigation',
        name: 'Smart Irrigation',
        tree: 'automation',
        cost: 32,
        seconds: 70,
        reveal: 'unlock-sensors',
        requires: 'unlock-adv-irrigation',
        gate: {kind: 'none'},
        grants: ['Sprinkler signal input', 'Signal wire endpoints on sprinklers'],
        blurb: 'Sprinklers gain a signal input. Unlocks Smart valve and Vehicle detector.',
        effect: {kind: 'feature'},
    },
    'unlock-chest': {
        id: 'unlock-chest',
        name: 'Chest',
        tree: 'utilities',
        cost: 12,
        seconds: 40,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Chest in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-chest'},
    },
    'unlock-contracts': {
        id: 'unlock-contracts',
        name: 'Contracts',
        tree: 'utilities',
        cost: 8,
        seconds: 30,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Contracts board at the stall', 'Reputation decay'],
        blurb: 'The stall can take orders from buyers.',
        effect: {kind: 'feature'},
    },
    'unlock-expand': {
        id: 'unlock-expand',
        name: 'Unlock land',
        tree: 'expansion',
        cost: 20,
        seconds: 40,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Land expansion on the map edge', '+1 expansion permit'],
        blurb: 'Unlocks land expansion on the map edge, and grants the first expansion permit.',
        effect: {kind: 'expand'},
    },
    'expand-land': {
        id: 'expand-land',
        name: 'Expand land',
        tree: 'expansion',
        cost: 100,
        seconds: 90,
        reveal: 'unlock-expand',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['+1 expansion permit'],
        blurb: 'A second expansion permit. Land still costs money on top of the permit.',
        effect: {kind: 'expand'},
    },
    'eminent-domain': {
        id: 'eminent-domain',
        name: 'Eminent domain',
        tree: 'expansion',
        cost: 400,
        seconds: 180,
        reveal: 'expand-land',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['+1 expansion permit'],
        blurb: 'A third expansion permit. Further permits are contract work, not paperwork.',
        effect: {kind: 'expand'},
    },
    'unlock-pickaxe': {
        id: 'unlock-pickaxe',
        name: 'Pickaxes',
        tree: 'utilities',
        cost: 0,
        seconds: 40,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Pickaxe and Hardened pickaxe in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-pickaxe'},
    },
    'unlock-fertilizer': {
        id: 'unlock-fertilizer',
        name: 'Synthetic fertilizer',
        tree: 'plants',
        cost: 5,
        seconds: 30,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Synthetic fertilizer in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-synth-fertilizer'},
    },
    'unlock-compost': {
        id: 'unlock-compost',
        name: 'Composting',
        tree: 'utilities',
        cost: 14,
        seconds: 45,
        reveal: 'unlock-fertilizer',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Compost box in the general store. Turns organic waste back into fertilizer.',
        effect: {kind: 'unlock-sku', sku: 'buy-compost-box'},
    },
    'unlock-grinder': {
        id: 'unlock-grinder',
        name: 'Seed grinder',
        tree: 'automation',
        cost: 18,
        seconds: 50,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Seed grinder and Mill in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-grinder'},
    },
    'unlock-preservatives': {
        id: 'unlock-preservatives',
        name: 'Preservatives',
        tree: 'automation',
        cost: 20,
        seconds: 55,
        reveal: 'unlock-grinder',
        requires: 'none',
        gate: {kind: 'none'},
        grants: ['Jam icon in the almanac'],
        blurb: 'Unlocks Jam machine, Freezer, and Sugar in the general store.',
        effect: {kind: 'unlock-sku', sku: 'buy-jam'},
    },
    'unlock-fermentation': {
        id: 'unlock-fermentation',
        name: 'Fermentation',
        tree: 'automation',
        cost: 14,
        seconds: 50,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Sugar cane seeds, Pot still, and Wine barrel. Ripe cane is fruit. Mill cane for sugar.',
        effect: {kind: 'unlock-sku', sku: 'pack-sugar-cane'},
    },
    'unlock-landscaping': {
        id: 'unlock-landscaping',
        name: 'Landscape architecture',
        tree: 'expansion',
        cost: 12,
        seconds: 60,
        reveal: 'start',
        requires: 'none',
        gate: {kind: 'none'},
        grants: [],
        blurb: 'Unlocks Grass seeds, Wooden fence and every paving tile in the general store.',
        effect: {kind: 'unlock-sku', sku: 'pack-grass'},
    },
}



export type SkuTab = 'seeds' | 'utility' | 'automation' | 'building'

/** `'prize'`: not for sale. Stock arrives from contract prizes and is spent on placement. */
export type SkuNeed = ResearchId | 'prize' | 'none'

export type Sku = {
    id: SkuId
    price: number
    tab: SkuTab
    unlock: 'start' | ResearchId
    show: 'start' | ResearchId
    need: SkuNeed
}

export const SKUS: { readonly [K in SkuId]: Sku } = {
    'pack-carrot': {id: 'pack-carrot', price: 3, tab: 'seeds', unlock: 'start', show: 'start', need: 'none'},
    'pack-potato': {id: 'pack-potato', price: 6, tab: 'seeds', unlock: 'start', show: 'start', need: 'none'},
    'pack-wheat': {id: 'pack-wheat', price: 10, tab: 'seeds', unlock: 'start', show: 'start', need: 'none'},
    'pack-tomato': {id: 'pack-tomato', price: 15, tab: 'seeds', unlock: 'unlock-tomato', show: 'start', need: 'none'},
    'pack-watermelon': {id: 'pack-watermelon', price: 18, tab: 'seeds', unlock: 'unlock-watermelon', show: 'start', need: 'none'},
    'pack-grape': {id: 'pack-grape', price: 16, tab: 'seeds', unlock: 'unlock-grape', show: 'start', need: 'none'},
    'pack-raspberry': {id: 'pack-raspberry', price: 22, tab: 'seeds', unlock: 'unlock-raspberry', show: 'unlock-grape', need: 'none'},
    'pack-sugar-cane': {id: 'pack-sugar-cane', price: 8, tab: 'seeds', unlock: 'unlock-fermentation', show: 'unlock-fermentation', need: 'none'},
    'buy-shovel': {id: 'buy-shovel', price: 10, tab: 'utility', unlock: 'start', show: 'start', need: 'none'},
    'buy-better-shovel': {id: 'buy-better-shovel', price: 30, tab: 'utility', unlock: 'unlock-better-tools', show: 'start', need: 'none'},
    'buy-pickaxe': {id: 'buy-pickaxe', price: 18, tab: 'utility', unlock: 'unlock-pickaxe', show: 'start', need: 'none'},
    'buy-better-pickaxe': {
        id: 'buy-better-pickaxe',
        price: 24,
        tab: 'utility',
        unlock: 'unlock-pickaxe',
        show: 'unlock-pickaxe',
        need: 'none',
    },
    'buy-bucket': {id: 'buy-bucket', price: 8, tab: 'utility', unlock: 'start', show: 'start', need: 'none'},
    'buy-bucket-large': {id: 'buy-bucket-large', price: 22, tab: 'utility', unlock: 'unlock-better-tools', show: 'start', need: 'none'},
    'buy-box': {id: 'buy-box', price: 6, tab: 'utility', unlock: 'start', show: 'start', need: 'none'},
    'buy-box-large': {id: 'buy-box-large', price: 18, tab: 'utility', unlock: 'unlock-large-box', show: 'start', need: 'none'},
    'buy-fertilizer': {id: 'buy-fertilizer', price: 18, tab: 'utility', unlock: 'start', show: 'start', need: 'none'},
    'buy-synth-fertilizer': {
        id: 'buy-synth-fertilizer',
        price: 15,
        tab: 'utility',
        unlock: 'unlock-fertilizer',
        show: 'unlock-fertilizer',
        need: 'none',
    },
    'buy-weed-spray': {
        id: 'buy-weed-spray',
        price: 12,
        tab: 'utility',
        unlock: 'unlock-fertilizer',
        show: 'unlock-fertilizer',
        need: 'none',
    },
    'buy-compost-box': {id: 'buy-compost-box', price: 20, tab: 'automation', unlock: 'unlock-compost', show: 'unlock-fertilizer', need: 'none'},
    'buy-pumpjack': {id: 'buy-pumpjack', price: 40, tab: 'automation', unlock: 'unlock-irrigation', show: 'start', need: 'none'},
    'buy-chest': {id: 'buy-chest', price: 18, tab: 'automation', unlock: 'unlock-chest', show: 'start', need: 'none'},
    'buy-grinder': {id: 'buy-grinder', price: 30, tab: 'automation', unlock: 'unlock-grinder', show: 'start', need: 'none'},
    'buy-pipe': {id: 'buy-pipe', price: 4, tab: 'automation', unlock: 'unlock-auto-irrigation', show: 'unlock-auto-irrigation', need: 'none'},
    'buy-sprinkler': {id: 'buy-sprinkler', price: 15, tab: 'automation', unlock: 'unlock-auto-irrigation', show: 'unlock-irrigation', need: 'none'},
    'buy-sprinkler-vert': {
        id: 'buy-sprinkler-vert',
        price: 30,
        tab: 'automation',
        unlock: 'unlock-adv-irrigation',
        show: 'unlock-auto-irrigation',
        need: 'none',
    },
    'buy-sprinkler-large': {
        id: 'buy-sprinkler-large',
        price: 33,
        tab: 'automation',
        unlock: 'unlock-adv-irrigation',
        show: 'unlock-auto-irrigation',
        need: 'none',
    },
    'buy-well': {id: 'buy-well', price: 75, tab: 'automation', unlock: 'unlock-adv-irrigation', show: 'unlock-auto-irrigation', need: 'none'},
    'buy-valve': {id: 'buy-valve', price: 6, tab: 'automation', unlock: 'unlock-auto-irrigation', show: 'unlock-auto-irrigation', need: 'none'},
    'buy-rain-tank': {id: 'buy-rain-tank', price: 20, tab: 'automation', unlock: 'unlock-auto-irrigation', show: 'unlock-irrigation', need: 'none'},
    'buy-tap': {id: 'buy-tap', price: 10, tab: 'automation', unlock: 'unlock-auto-irrigation', show: 'unlock-irrigation', need: 'none'},
    'buy-tile-cobble': {id: 'buy-tile-cobble', price: 5, tab: 'building', unlock: 'unlock-landscaping', show: 'start', need: 'none'},
    'buy-tile-brick': {id: 'buy-tile-brick', price: 7, tab: 'building', unlock: 'unlock-landscaping', show: 'start', need: 'none'},
    'buy-tile-paved': {id: 'buy-tile-paved', price: 11, tab: 'building', unlock: 'unlock-landscaping', show: 'start', need: 'none'},
    'buy-fence': {id: 'buy-fence', price: 10, tab: 'building', unlock: 'unlock-landscaping', show: 'start', need: 'none'},
    'pack-grass': {id: 'pack-grass', price: 1, tab: 'seeds', unlock: 'unlock-landscaping', show: 'start', need: 'none'},
    'buy-mill': {id: 'buy-mill', price: 35, tab: 'automation', unlock: 'unlock-grinder', show: 'start', need: 'none'},
    'buy-jam': {id: 'buy-jam', price: 40, tab: 'automation', unlock: 'unlock-preservatives', show: 'unlock-grinder', need: 'none'},
    'buy-still': {id: 'buy-still', price: 45, tab: 'automation', unlock: 'unlock-fermentation', show: 'start', need: 'none'},
    'buy-barrel': {id: 'buy-barrel', price: 28, tab: 'automation', unlock: 'unlock-fermentation', show: 'start', need: 'none'},
    'buy-freezer': {id: 'buy-freezer', price: 36, tab: 'automation', unlock: 'unlock-preservatives', show: 'unlock-grinder', need: 'none'},
    'buy-freezer-large': {id: 'buy-freezer-large', price: 0, tab: 'automation', unlock: 'start', show: 'start', need: 'prize'},
    'buy-sugar': {id: 'buy-sugar', price: 16, tab: 'utility', unlock: 'unlock-preservatives', show: 'unlock-grinder', need: 'none'},
    'buy-hangar': {id: 'buy-hangar', price: 80, tab: 'automation', unlock: 'unlock-vehicles', show: 'unlock-irrigation', need: 'none'},
    'buy-silo-seed': {id: 'buy-silo-seed', price: 70, tab: 'automation', unlock: 'unlock-vehicles', show: 'unlock-irrigation', need: 'none'},
    'buy-silo-spray': {id: 'buy-silo-spray', price: 70, tab: 'automation', unlock: 'unlock-vehicles', show: 'unlock-irrigation', need: 'none'},
    'buy-silo-produce': {id: 'buy-silo-produce', price: 70, tab: 'automation', unlock: 'unlock-vehicles', show: 'unlock-irrigation', need: 'none'},
    'buy-lever': {id: 'buy-lever', price: LEVER_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-button': {id: 'buy-button', price: BUTTON_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-lamp': {id: 'buy-lamp', price: LAMP_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-or': {id: 'buy-or', price: OR_PRICE, tab: 'automation', unlock: 'unlock-advanced-sensors', show: 'unlock-sensors', need: 'unlock-sensors'},
    'buy-and': {id: 'buy-and', price: AND_PRICE, tab: 'automation', unlock: 'unlock-advanced-sensors', show: 'unlock-sensors', need: 'unlock-sensors'},
    'buy-not': {id: 'buy-not', price: NOT_PRICE, tab: 'automation', unlock: 'unlock-advanced-sensors', show: 'unlock-sensors', need: 'unlock-sensors'},
    'buy-pulser': {id: 'buy-pulser', price: PULSER_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-counter': {id: 'buy-counter', price: COUNTER_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-sensor-water': {id: 'buy-sensor-water', price: SENSOR_WATER_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-sensor-fert': {id: 'buy-sensor-fert', price: SENSOR_FERT_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-sensor-harvest': {id: 'buy-sensor-harvest', price: SENSOR_HARVEST_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-sensor-day': {id: 'buy-sensor-day', price: SENSOR_DAY_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-water-system': {id: 'buy-water-system', price: WATER_SYSTEM_PRICE, tab: 'automation', unlock: 'unlock-sensors', show: 'unlock-sensors', need: 'none'},
    'buy-smart-valve': {
        id: 'buy-smart-valve',
        price: SMART_VALVE_PRICE,
        tab: 'automation',
        unlock: 'unlock-smart-irrigation',
        show: 'unlock-sensors',
        need: 'unlock-sensors',
    },
    'buy-vehicle-detector': {
        id: 'buy-vehicle-detector',
        price: VEHICLE_DETECTOR_PRICE,
        tab: 'automation',
        unlock: 'unlock-smart-irrigation',
        show: 'unlock-sensors',
        need: 'unlock-sensors',
    },
}
