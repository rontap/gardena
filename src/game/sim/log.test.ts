// COMMANDMENT: never test specifically for versions, ever. expect(SAVE_VERSION) or PROTOCOL .toBe is disallowed.
import { describe, expect, test } from 'vitest'
import { Act, type Cmd } from './log.ts'

describe('0.9 log', () => {
  test('JSON.parse(JSON.stringify(cmd)) equals that Cmd arm', () => {
    const cmds: Cmd[] = [
      { a: Act.click, t: 0, p: 0, c: [1, 2] },
      { a: Act.clickValve, t: 1, p: 0, e: { axis: 'h', col: 0, row: 0 } },
      { a: Act.enqueue, t: 2, p: 0, i: { act: 'walk', at: { col: 1, row: 2 } } },
      { a: Act.enqueue, t: 2, p: 0, i: { act: 'valve', at: { col: 1, row: 2 }, edge: { axis: 'v', col: 1, row: 2 } } },
      { a: Act.buy, t: 3, p: 0, s: 'pack-carrot', c: [0, 0] },
      { a: Act.buyPacks, t: 4, p: 0, s: 'pack-carrot', c: [0, 0] },
      { a: Act.placePipe, t: 5, p: 0, e: { axis: 'v', col: 1, row: 1 } },
      { a: Act.placeSprinkler, t: 6, p: 0, s: { variant: 'basic', at: { col: 1, row: 1 }, tune: { kind: 'flat' }, inn: 0, hold: 0 } },
      { a: Act.placeSprinkler, t: 6, p: 0, s: { variant: 'vert', at: { col: 2, row: 2 }, facing: 'ew', tune: { kind: 'crop', crop: 'wheat' }, inn: 0, hold: 0 } },
      { a: Act.placeSprinkler, t: 6, p: 0, s: { variant: 'large', at: { col: 3, row: 3 }, tune: { kind: 'flat' }, inn: 0, hold: 0 } },
      { a: Act.delete, t: 7, p: 0, k: 'pipe', e: { axis: 'h', col: 2, row: 2 } },
      { a: Act.delete, t: 8, p: 0, k: 'sprinkler', c: [3, 3] },
      { a: Act.delete, t: 9, p: 0, k: 'building', c: [4, 4] },
      { a: Act.expand, t: 10, p: 0, k: { cx: 1, cy: 0 } },
      { a: Act.startResearch, t: 11, p: 0, r: 'unlock-expand' },
      { a: Act.pickSkill, t: 12, p: 0, id: 'boots' },
      { a: Act.sellAll, t: 13, p: 0 },
      { a: Act.swap, t: 15, p: 0, i: 0 },
      { a: Act.swapChest, t: 16, p: 0, c: [5, 5], i: 1 },
      { a: Act.tuneSprinkler, t: 17, p: 0, c: [6, 6], u: { kind: 'crop', crop: 'carrot' } },
      { a: Act.openHud, t: 18, p: 0, k: 'sprinkler', c: [7, 7] },
      { a: Act.armWire, t: 38, p: 0, from: { kind: 'cell', at: { col: 1, row: 1 }, port: 'out' } },
      { a: Act.placeWire, t: 39, p: 0, from: { kind: 'cell', at: { col: 1, row: 1 }, port: 'out' }, to: { kind: 'cell', at: { col: 1, row: 2 }, port: 'in' } },
      { a: Act.tuneWater, t: 41, p: 0, c: [3, 3], wilt: true, over: false },
      { a: Act.tuneHarvest, t: 42, p: 0, c: [4, 4], mode: 'all' },
      { a: Act.tuneCounter, t: 48, p: 0, c: [5, 5], n: 10 },
      { a: Act.resetCounter, t: 52, p: 0, c: [5, 5] },
      { a: Act.openHud, t: 49, p: 0, k: 'counter', c: [5, 5] },
      { a: Act.openHud, t: 50, p: 0, k: 'day', c: [6, 6] },
      { a: Act.tuneDay, t: 51, p: 0, c: [6, 6], sunrise: true, day: false, sunset: false, twilight: false },
      { a: Act.delete, t: 43, p: 0, k: 'wire', from: { kind: 'cell', at: { col: 1, row: 1 }, port: 'out' }, to: { kind: 'cell', at: { col: 1, row: 2 }, port: 'in' } },
      { a: Act.closeHud, t: 19, p: 0 },
      { a: Act.armDelete, t: 20, p: 0 },
      { a: Act.cancelPlace, t: 21, p: 0 },
      { a: Act.rotatePlace, t: 22, p: 0 },
      { a: Act.dismissRecap, t: 23, p: 0 },
      { a: Act.ackCue, t: 24, p: 0 },
      { a: Act.rightClick, t: 25, p: 0, c: [8, 8] },
      { a: Act.cheat, t: 26, p: 0, k: 'all' },
      { a: Act.cheat, t: 27, p: 0, k: 'money' },
      { a: Act.cheat, t: 28, p: 0, k: 'points' },
      { a: Act.cheat, t: 29, p: 0, k: 'research' },
      { a: Act.cheat, t: 62, p: 0, k: 'speed', n: 1 },
      { a: Act.cheat, t: 63, p: 0, k: 'speed', n: 3 },
      { a: Act.cheat, t: 64, p: 0, k: 'day' },
      { a: Act.cheat, t: 65, p: 0, k: 'skills' },
      { a: Act.cheat, t: 66, p: 0, k: 'produce' },
      { a: Act.drive, t: 30, p: 0, throttle: 1, steer: -1 },
      { a: Act.stride, t: 47, p: 0, x: 1, y: -1 },
      { a: Act.buyVehicle, t: 31, p: 0, c: [1, 2], k: 'quad' },
      { a: Act.buyTrailer, t: 31, p: 0, c: [1, 2], k: 'seed' },
      { a: Act.deploy, t: 32, p: 0, v: 1, c: [3, 4], hitch: 'none' },
      { a: Act.swapTrailer, t: 32, p: 0, u: 1, i: 0 },
      { a: Act.embark, t: 33, p: 0, v: 1 },
      { a: Act.disembark, t: 34, p: 0 },
      { a: Act.dock, t: 35, p: 0 },
      { a: Act.swapVehicle, t: 36, p: 0, v: 1, i: 2 },
      { a: Act.refill, t: 37, p: 0, c: [5, 6] },
      { a: Act.setBoom, t: 38, p: 0, w: 3 },
      { a: Act.load, t: 45, p: 0 },
      { a: Act.unload, t: 46, p: 0 },
      { a: Act.acceptContract, t: 53, p: 0, c: 4 },
      { a: Act.cancelContract, t: 54, p: 0, c: 4 },
      { a: Act.reorderContract, t: 55, p: 0, c: 4, d: -1 },
      { a: Act.route, t: 56, p: 0, k: 'create' },
      { a: Act.route, t: 56, p: 0, k: 'delete', r: 1 },
      { a: Act.route, t: 56, p: 0, k: 'setDeploy', r: 1, d: { kind: 'quad' } },
      { a: Act.route, t: 56, p: 0, k: 'setDeploy', r: 1, d: { kind: 'tractor', trailer: 'seed', boom: 3 } },
      { a: Act.route, t: 57, p: 0, k: 'add', r: 1, s: { kind: 'goto', at: { col: 1, row: 2 } } },
      { a: Act.route, t: 57, p: 0, k: 'add', r: 1, s: { kind: 'wait', at: { col: 3, row: 4 } } },
      { a: Act.route, t: 58, p: 0, k: 'remove', r: 1, i: 0 },
      { a: Act.route, t: 58, p: 0, k: 'move', r: 1, i: 0, s: { kind: 'goto', at: { col: 5, row: 6 } } },
      { a: Act.route, t: 58, p: 0, k: 'reorder', r: 1, i: 0, to: 2 },
      { a: Act.route, t: 59, p: 0, k: 'rename', r: 1, n: 'Loop' },
      { a: Act.route, t: 60, p: 0, k: 'deploy', r: 1 },
      { a: Act.route, t: 60, p: 0, k: 'recall', v: 1 },
      { a: Act.route, t: 61, p: 0, k: 'automate', v: 1, c: [10, 12] },
    ]
    cmds.forEach(cmd => {
      expect(JSON.parse(JSON.stringify(cmd))).toEqual(cmd)
    })
  })

  test('`Act.setBoom` `\'W\'`; `Act.placeWire` `\'N\'`; `Act.load` `\'L\'`; `Act.unload` `\'U\'`; `Act.stride` `\'K\'`; `Act.tuneCounter` `\'M\'`; `Act.tuneDay` `\'O\'`; `Act.acceptContract` `\'J\'`; `Act.cancelContract` `\'Y\'`; `Act.reorderContract` `\'Z\'`; `Act.route` `\'o\'`; inner `k` closed union `create` | `delete` | `assign` | `add` | `remove` | `reorder` | `rename` | `start` | `automate`. Latest same-`t` `assign` / `start` wins like drive. Spray click is `Intent` `{ act: \'weed-spray\'; at }`. `Act.cheat` `\'u\'`; inner `k` closed union `all` | `money` | `points` | `research` | `speed` | `day` | `skills` | `produce`. `speed` carries `n: 1 | 3`.', () => {
    expect(Act.setBoom).toBe('W')
    expect(Act.placeWire).toBe('N')
    expect(Act.load).toBe('L')
    expect(Act.unload).toBe('U')
    expect(Act.stride).toBe('K')
    expect(Act.tuneCounter).toBe('M')
    expect(Act.tuneDay).toBe('O')
    expect(Act.acceptContract).toBe('J')
    expect(Act.cancelContract).toBe('Y')
    expect(Act.reorderContract).toBe('Z')
    expect(Act.route).toBe('o')
    expect(Act.cheat).toBe('u')
  })
})
