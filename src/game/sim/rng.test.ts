...
describe('0.9 rng', () => {
  test('Spatial.at / hash: same args, any call order → same u', () => {
    ...
    expect(b.stream('grow').at(10, 12, 1, 0)).toBe(grow)
    expect(a.stream('grow').at(10, 12, 1, 1)).not.toBe(grow)
  })
})
