...
- 🔧 Changed ui: Research. previously, cards sat in a grid under category tabs, now they sit in paths from left to right, and you click a card then Research, or double-click to start.
- 🔧 Changed ui: Family. previously, each person had a column of skills, now the cards sit in paths like Research, with Reputation and Luck above, and you click a card then Get skill.
- 🔧 Changed mechanic: Pulser and Counter now require Advanced sensors. Day sensor and Weather sensor now require Smart Irrigation Sensors. Traffic light now requires Automated dispatch. Variety sensor now requires Crop variants.
- 🔧 Changed mechanic: Driving classes now requires Machinery. Industrial farmer now requires Broker skill.
...
- ✨ Added mechanic: Tree Grafting. A chop drops grafts only after you choose this skill.
...
- 🎉 Added mechanic: Necronomicon three new pages: gather three Fly agaric, then a Rotary shovel or a Diamond pickaxe, then a supper of one Barackpálinka, one Premium wine and one Bread.
- ✨ Added mechanic: Picking up spoiled crops. An empty hand on Rotten produce or a Dead plant picks one up and leaves tilled soil behind, where before a shovel was the only way to clear the plot and nothing came off it.
- 🔧 Changed item: Rotten produce. Nerfed value in compost box from 2 to 1.
- 🔧 Changed mechanic: Rotten produce on the ground. After three days, it rots off completely.
- 🔧 Changed mechanic: Tree. Nerfed on-season yield from 350% to 300%, and nerfed off-season yield to 300%.
- 🔧 Changed item: Cherry and Apricot tree, nerfed growth speed by 20 seconds.
- 🔧 Changed mechanic: Furnace. Now a furnace does not buff itself.
- ✨ Added ui: Reputation on a finished contract. Every contract history line, on Contracts and in the end-of-day summary, says how much Reputation that contract gained or cost, where before only the bar above the board moved.
- 🔧 Changed mechanic: Nerfed burrow appearance chance from 75% to 66%.
- 🔧 Changed item: Treasure. previously, the money in a burrow was nerfed.

# 2.8.0 The Necronomicon

Added endgame item: Necronomicon. The necronomicon unlocks mid-game and requires increasingly complex sacrafices to it.

- 🎉 Added mechanic: Necronomicon three new pages: gather three Fly agaric, then a Rotary shovel or a Diamond pickaxe, then a supper of one Barackpálinka, one Premium wine and one Bread.
# 2.7.3 Better build


 Reworked Build menu, All buildings can be deleted, and cancel button is visible whenever a building is selected. When deleting a building, normal soil is shown instead of tilled soil. Placement preview now shows chest inputs and outputs, and vehicle interactions - Aron 
...
- 🐛 Fixed bug ui: Seed Variety Station, Paving. Can be now demolished.
...
Grass seeds can be now managed in the Seed silo, like regular seeds.

- 🔧 Changed item: Grass seeds. Moved from build to seed silo.
- 🐛 Fixed bug ui: Chest beside Mill, Infuser, or Furnace had its connector on the upper tile, moved to the lower tile.
...
- 🔧 Changed ui: Stipend renamed to Support from grandma. previously, the extra money at the end of the day stayed the same size, now it gets smaller over the first days and then stops.
...
# 2.7.1 Multiplayer Update II

Fixed bugs leading to desyncs in multiplayer. Command Center names other gardeners when they join, leave, or their farm drifted.
...
- 🐛 Fixed bug multiplayer: Wooden fence. The host dragging Wooden fence used to spend money only on the host, so the guest's farm drifted and they were sent back to the main menu.
...
- 🎉 Added ui: Player joined, left or drifted. Command Center shows that gardener's name when they arrive, leave or desync.

# 2.7.0 Infusion

New Building introduced: infuser. Infuser takes a product, like am, wine, cider, spirits, or Olive oil and infuses it with Vanilla extract or chilli flakes. Infused goods sell at Market without changing that good's saturation, and finishing a contract with them raises Reputation more than the same contract without them. 

- 🎉 Added mechanic: Infusion. Infuse jam, wine, cider, spirits, or Olive oil at an Infuser with Flakes or Vanilla extract, one of those two, not both.
...
- 🐛 Fixed bug ui: Seed silo and Additive store. previously, taking a second crop or additive while your hand was full put the first one on the ground, now the seeds in your hand go back on the shelf and the crop you clicked comes out, and clicking the crop you are already holding adds the two together.
- 🔧 Changed mechanic: Weed spray. Now takes 0.5 seconds.
...
Merged the general store and the build menu. Seeds and additives can be instead bought directly in the seed silo. Tools moved to build menu.
...
- 🎉 Added mechanic: Hold Shift  as you click keeps what you are placing in hand, so a row of chests is one pick and many clicks instead of a trip back to the Build menu for each one.
- 🔧 Changed ui: Almanac, cheat. previously, it opened from the left rail, now it opens from a button on the top bar beside Pause.
- 🎉 Added ui: Reputation. A gold star on Family shows what the companies think of your farm.
- 🎉 Added ui: Luck. A four-leaf clover on Family shows your Luck.
- 🔧 Changed ui: Market. previously, it opened on Stall every time, now it opens on the tab you last read.
- 🎉 Added mechanic: Auto-restock. A Seeding silo and an Additive silo each gain a tick box; with it on, whatever you take out of that silo is bought immediately.
- 🔧 Changed mechanic: end of day. previously, a new day paused the farm, now the farm keeps running.

# 2.6.3 Research rebalance

Rebalanced research items, and added Chainsaw.

- ✨ Added item: Chainsaw. Upgraded version of axe, faster and more durable.
- 🎉 Added research: Hardened tools. Unlocks Chainsaw and hardened pickaxe
- 🔧 Changed item: Axe. Nerfed durability 40 -> 30.
- 🔧 Changed research: Synthetic additives. previously, this research was named Synthetic fertilizer, now it is named Synthetic additives.  Gardening tools. previously, this research was named Better gardening tools and sat in Trade, now it is named Gardening tools and sits in Plants. Seed Variety Station. previously, it sat on Processing from the first day, now it shows after Crop variants. Landscape architecture. previously, Expansion had to show first, now it sits on Land from the start.  Pot still and furnace. previously, it sat on Processing from the first day, now it shows after Machinery. Machinery skill. previously, Husband could learn it from the first day, now it is offered after Machinery research.

# 2.6.2 Command center update II

The right-hand column is titled Command Center. Ending a day no longer pauses the game. Added loading screen.
...
- 🔧 Changed ui: end-of-day summary. previously, ending a day held the farm on a screen you had to dismiss before the next day would continue. Now the game continues, and a command center notification shows up instead.
- 🎉 Added ui: Loading.... While the map is still coming up, Loading... sits in the middle of the farm.

# 2.6.0 Command Center

New feature: Current events are shown in the command center in the right pane of the game. Research, Skills, Expansion points are all moved here. Command center displays plants wilting, losing freshness,  needing fertilizer and being overwatered. - Aron
...
Sensors can be built around fences, and when a fence is enclosed, the sensor's range matches the fenced area. Restructured how sensors work. OR and AND gate have been merged to logic gates. Two new sensors: Variety detecting plant variety and Weather sensor, tunable to signal current weather. Pressure plate (renamed from vehicle detector) can detect the players or items on the ground. 
...
- 🚫 Removed building: OR / AND gate. Buy a Logic gate and set OR / AND.
...
- 🔧 Changed building: Pump. Can be turned off with a signal.
- 🎉 Added mechanic: Fenced sensors. A Water sensor, Fertilizer sensor, Harvest sensor, Variety sensor, or Pressure plate on a Wooden fence watches the plots inside that fence. 
...
- 🔧 Changed ui: Recipes and Inspect. previously, litre amounts and plot water printed two decimals, now they round to the nearest half litre.

# 2.4.4 Sails and Silos

The three field silos finally hold what their names promise, and paving stays put under whatever you build on it. Redesigned Mill to be a 2×2 building.   - Aron
...
- 🔧 Changed building: Mill. previously, it was a one-tile building, now it is a two-by-two windmill with a tapered stone tower and four sails.
...
- 🎉 Added ui: VFX for Mill, Seed grinder, sprinklers, tractor, pumpjack and Seed variety station machines.
- 🔧 Redesigned buildings: Well, rainwater tank, tap, bucket, paving slab and burrow. - Aron

# 2.4.3 Burrows

Holes in the ground hold treasure, seeds, and tools. Dig them with a shovel. Lucky on Family makes later burrows better.
...
- 🎉 Added mechanic: Burrow. A hole in untilled ground that you Dig with a shovel to drop what it holds, and digging it does not till the ground.
  ...
  - You cannot place, pave, fence, or plant a tree on it. 
- ✨ Added item: Treasure. You open it on a plot you own for the money it hold.
...
# 2.4.1 Variety II & QoL V

Heirloom fruits have their own unique products, with names and containers to match. The inspect panel reads Quality and Freshness off bars.
...
- ✨ Added item: Premium casks. previously, a barrel filled with an Heirloom Variety poured the same Wine or Cider as any other Variety, now it pours Premium wine or Premium cider, and the name follows the bottle from the barrel to the stall.
- ✨ Added item: Grape jelly. previously, it drew the plain grape jam jar, now it draws a taller jar under a gold wax cap with a label of its own.
- ✨ Added item: Black raspberry jam. previously, it drew the plain raspberry jam jar, now it draws a jar under a tied cloth lid, filled almost black.
- ✨ Added item: Passata. previously, it drew the ketchup bottle, now it is a can with a label of its own.
- ✨ Added item: Barackpálinka. previously, it read Apricot pálinka and drew the Brandy bottle, now it reads Barackpálinka and draws a slim gold-foiled bottle nothing else uses.
...
Rarity is removed from the game and split into Variety and Quality. Every plant now carries a Variety it was sown with and a Quality it earned, grafts move a Variety onto a plant already growing, and the machines pass both into what they make.
...
- 🔧 Changed ui: Machine inspect. Now hovering a Mill, Jam machine, Pot still, Seed grinder, Compost box, or Furnace shows it finishes faster with that working Furnace than without a Furnace.

# 2.2.0 Settings

Added settings menu to the game, redesigned pause menu

- 🎉 Added ui: Settings. A page behind the gear holds a reduced-motion switch and a switch that stops the farm while this tab is not in front.
- 🎉 Added ui: Main menu in the settings menu. A button in the gear saves the farm and takes you back to the startup screen.
...
- 🔧 Changed ui: Weed spray. previously, the can had no nozzle, now it shows a hose and spray nozzle.
...
- 🔧 Changed building: Pot still. Water consumption increased from 1L to 2L.
...
- 🔧 Changed mechanic: Pump water. Price increased from 20$ per day to 40$ per day, if used constantly.
- 🔧 Changed ui: Money. previously, prices split into gold and silver coins, now every price shows as a single rounded coin.
...
- 🔧 Changed mechanic: Careful tending. now you can also tend an off-season tree once, which raises the next on-season chance by 15%.
- 🔧 Changed building: Cider requires 4 apples instead of 5.
- 🔧 Changed building: Seed grinder. Nerfed speed from 5 seconds to 12 seconds.
...
- 🔧 Changed ui: Almanac. previously olive sat with the annuals, now it sits with the trees. . previously a fruit page showed jam as its third picture, now it shows the jam, spirit, wine, cider, or oil that fruit makes.
...
- 🔧 Changed building: Mill. Two vanilla fruits mill into three extract.

# 2.0.4 Rebalance

Rarity requires on Crop variants research. Shop prices and machine times move.
...
- 🚫 Removed research: Composting. The Compost box is in the store from the first day.
...
Replaced game engine, game runs much faster and does not slow down.
...
Fruit, seeds, and weeds stack in your hand. Fruit boxes are removed from the game.
...
Quad and tractor can be automated. A traffic light holds a truck only when that light is on the list.
...
- 🔧 Changed researches: Pickaxe. previously, its project was free, now it costs money and waits until there is rock worth breaking. Rainwater tank. previously, it needed a research project, now it is in the store from the first day. Sensors. previously, was locked behind irrigation and sold a water reader with no plumbing to read, now it is open from the start and the readers that need plumbing, soil work or vehicles wait for those instead. Smart irrigation. previously, the sprinkler crop dial and the sprinkler signal input were two projects, now one project grants both. Composting. previously, it sat behind Synthetic fertilizer, now the two cost the same and sit side by side from the first day.
...
- 🚫 Removed item from shop: Vanilla seeds. The shop no longer sells them, and only a contract prize hands them over.
- 🚫 Removed mechanic: Vanilla tending. That skill is gone.
- 🚫 Removed item from shop: Rotary shovel. The shop no longer sells it, and a four-star contract can hand it over.
- 🚫 Removed item from shop: Diamond pickaxe. The shop no longer sells it, and a four-star contract can hand it over.
...
- 🚫 Removed item: Lemon, lemon fruits and lemon trees.
...
Loops can be added to sensor networks.
...
A pulser, a counter, and a day sensors are added. A wire can flip a lever. 
...
- 🔧 Changed building: AND / OR / NOT. previously, Sensors unlocked it, now it waits on Advanced sensors.
...
- 🔧 Changed mechanic: Vehicles. Speed is nerfed.
...
- 🔧 Changed building: Mill, Jam machine. previously, it always took input, now a wire can disable its input.
...
- 🔧 Changed building: Chest, Freezer, Seed silo, Additive silo. previously, it had no signal, now it outputs when full.
...
- 🔧 Changed building: AND / OR. previously, it was larger, now it is more compact.
...
- 🔧 Changed item: Fertilizer bag, Bucket. Capacity has been buffed.
...
- ✨ Added building: Seeding silo, Spraying silo, Produce Silo. An inert building.
...
- ✨ Added item: Cobble, Brick, Paved ground. Decorative items.
...
Added family, and skill points to the game.
...
Picked fruit goes stale. Fruit rarity is added.
...
Pipes and sprinklers water the soil.
...
- 🔧 Changed ui: Shop, Research, Market. previously, it was not a side panel, now it is.
