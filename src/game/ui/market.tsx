...
import { STALL_IDS, type StallSale } from '../sim/stall.ts'
  ...
  return (
    ...
    >
      <Chrome className="relative max-h-[calc(100%-4rem)] w-[40rem] overflow-y-auto px-4 pb-5 pt-7">
        ...
        <svg viewBox="0 0 240 120" aria-hidden="true" className="relative z-20 h-24 w-full" dangerouslySetInnerHTML={{ __html: UI_MARKET_STALL }} />
...
function StallRow({ id, world }: { id: StallGoodId; world: World }) {
  return (
    <div data-stall-box={id} className="flex items-center gap-3 bg-dirt px-2 py-2 text-sm">
      <ItemFace item={boxFace(id)} />
      <span>{stockCount(world, id)}</span>
    </div>
  )
}
...
function stockCount(world: World, id: StallGoodId): number {
  return RARITY_RANK.reduce((n, rarity) => n + world.stall[id].stock[rarity], 0)
}
...
function DynamicStallRow({ id, world, toasts }: { id: StallGoodId; world: World; toasts: readonly StallSale[] }) {
  ...
  return (
    <div data-stall-box={id} className="relative flex items-center gap-2 bg-dirt px-2 py-2 text-sm">
      <ItemFace item={boxFace(id)} />
      <div>{stockCount(world, id)}</div>
      ...
      {RARITY_RANK.slice(1).filter(r => good.stock[r] > 0).map(r => <svg key={r} viewBox="0 0 8 8" dangerouslySetInnerHTML={{ __html: qualityPip(r) as string }} />)}
    </div>
  )
}
