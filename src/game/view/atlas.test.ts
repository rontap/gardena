...
test('view.groups — every `<g id>` the atlas asks for exists in the file it reads, and the file carries no group the atlas never asks for. Missing group is a permanent atlas boot failure; a spare one is art nobody can reach.', () => {
  ...
  expect(groupIds('props/prop-logic.svg')).toEqual(['or', 'and'])
  expect(groupIds('items/item-logic.svg')).toEqual(['or', 'and'])
  expect(groupIds('props/prop-sensor-variety.svg')).toEqual(['off', 'on'])
  expect(groupIds('items/item-sensor-variety.svg')).toEqual(['off', 'on'])
